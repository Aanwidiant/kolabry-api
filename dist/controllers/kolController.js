...
import { NicheType } from "@prisma/client";
import { Pagination } from "../helpers/pagination.js";
// Create KOL data
export const createKol = async (c) => {
    try {
        ...
        for (const [index, item] of dataArray.entries()) {
            ...
            try {
                ...
                results.push({
                    success: false,
                    message: `Item ${index + 1}: Failed to create KOL data. ${error instanceof Error ? error.message : String(error)}`,
                });
            }
        }
        return c.json({
...
// Get KOL data
export const getKols = async (c) => {
    try {
        const { search = "", page = "1", limit = "10", niche } = c.req.query();
        const currentPage = parseInt(page, 10);
        const take = parseInt(limit, 10);
        const skip = (currentPage - 1) * take;
        const isValidNiche = (value) => Object.values(NicheType).includes(value);
        const whereClause = {
            ...(niche && isValidNiche(niche) ? { niche: niche } : {}),
            ...(search
                ? {
                    name: {
                        contains: search,
...
// Update KOL data
export const updateKol = async (c) => {
    try {
        const body = await c.req.json();
        if (!body.id) {
            return c.json({
...
// Delete KOL data
export const deleteKol = async (c) => {
