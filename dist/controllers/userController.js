...
export const updateUser = async (c) => {
    ...
    if (role !== undefined) {
        if (user.role === "ADMIN") {
            const allowedRoles = ["KOL_MANAGER", "ADMIN", "BRAND"];
...
export const changePassword = async (c) => {
    ...
    if (!validatePasswordContain.test(newPassword)) {
        return c.json({
            success: false,
            message: "The new password must have at least 8 characters, contain uppercase letters, lowercase letters, numbers, and special characters.",
        }, 400);
    }
